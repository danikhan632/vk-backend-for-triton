...
static spv_target_env defaultSPIRVTargetEnv = SPV_ENV_VULKAN_1_1;
...
static LogicalResult getInterfaceVariables(spirv::FuncOp funcOp, SmallVectorImpl<Attribute> &interfaceVars) {
    std::cout << "Entered getInterfaceVariables" << std::endl;

    auto module = funcOp->getParentOfType<spirv::ModuleOp>();
    if (!module) {
      std::cerr << "Error: Failed to find parent module of the function." << std::endl;
      return failure();
    }
    
    SetVector<Operation *> interfaceVarSet;
    unsigned int descriptorCounter = 0;  // Counter for descriptor sets

    funcOp.walk([&](spirv::AddressOfOp addressOfOp) {
      auto var = module.lookupSymbol<spirv::GlobalVariableOp>(addressOfOp.getVariable());
      if (!var) {
        std::cerr << "Failed to lookup symbol for variable: " << addressOfOp.getVariable().str() << std::endl;
        return;
      }
      ...
      case spirv::StorageClass::Image: {
        // If descriptor_set attribute does not exist, set it using descriptorCounter
        if (!var.getDescriptorSet().has_value()) {
            auto intType = IntegerType::get(funcOp.getContext(), 32);
            var.getOperation()->setAttr("descriptor_set", IntegerAttr::get(intType, descriptorCounter));
        }

        if (!var.getBinding().has_value()) {
            // If binding attribute does not exist, set it to 0 or some default value
            auto intType = IntegerType::get(funcOp.getContext(), 32);
            var.getOperation()->setAttr("binding", IntegerAttr::get(intType, 0));
        }
      ...
      std::cout << "\033[1;34m" << "Descriptor counter value: " << (descriptorCounter) << "\033[0m" << std::endl;

    });

    std::cout << "Function being processed:" << std::endl;
    // funcOp.dump();

    std::cout << "Total interface variables found: " << interfaceVarSet.size() << std::endl;

    for (auto &var : interfaceVarSet) {
      auto symName = cast<spirv::GlobalVariableOp>(var).getSymName();
      interfaceVars.push_back(SymbolRefAttr::get(funcOp.getContext(), symName));
      std::cout << "Added to interfaceVars: " << symName.str() << std::endl;
    }

    // Printing contents of interfaceVars
    std::cout << "Contents of interfaceVars: " << std::endl;
    for (const auto &attr : interfaceVars) {
        if (auto symRefAttr = attr.dyn_cast<SymbolRefAttr>()) {
            std::cout << symRefAttr.getRootReference().str() << std::endl;
        }
    }
...
static bool optimizeSPIRVModule(std::vector<uint32_t> &binary) {
  ...
  auto runOptimizer = [&](const std::vector<std::string> &flags) -> bool {
    llvm::errs() << "Registering passes for optimization...\n";
    if (!optimizer.RegisterPassesFromFlags(flags)) {
      llvm::errs() << "Failed to register passes.\n";
      return false;
    }

    llvm::errs() << "Executing optimization...\n";
    std::vector<uint32_t> optimized;
    if (!optimizer.Run(binary.data(), binary.size(), &optimized, optimizer_options)) {
      llvm::errs() << "Optimization execution failed.\n";
      return false;
    }

    if (binary == optimized) {
      llvm::errs() << "Binary remains unchanged after optimization.\n";
    } else {
      llvm::errs() << "Binary modified during optimization.\n";
    }
  ...
  llvm::errs() << "Starting first optimization stage...\n";
  ...
  llvm::errs() << "Starting second optimization stage...\n";
  if (!runOptimizer({"--eliminate-dead-const"})) {
    return false;
  }

  llvm::errs() << "Optimization complete.\n";
  return true;
}
...
static LogicalResult translateTritonSPIRVToSPIRVIR(ModuleOp module, raw_ostream &output) {
  ...
  module.walk([&](ModuleOp op) {
      auto newModuleOp = builder.create<spirv::ModuleOp>(op.getLoc(), op.getName());

      unsigned threadsPerWarp = triton::gpu::TritonGPUDialect::getThreadsPerWarp(op);
      ...
      newModuleOp->setAttr("addressing_model", builder.getAttr<spirv::AddressingModelAttr>(spirv::AddressingModel::Logical));

      newModuleOp->setAttr("memory_model",builder.getAttr<spirv::MemoryModelAttr>(spirv::MemoryModel::Vulkan));
  ...
  if (spirvModules.empty()) return module.emitError("found no 'spv.module' op");

  if (spirvModules.size() != 1) return module.emitError("found more than one 'spv.module' op");
  ...
  for (auto &sprivModule : spirvModules) {
        ...
        auto entryPointAttr = op->getAttrOfType<spirv::EntryPointABIAttr>(entryPointAttrName);
        llvm::errs() << "\033[1;34m" << entryPointAttr << "\033[0m\n";

        if (!entryPointAttr) {return;}
        op.setName("main");
        OpBuilder::InsertionGuard moduleInsertionGuard(builder);
        auto spirvModule = op->getParentOfType<spirv::ModuleOp>();
        // spirvModule.getBody()->print(llvm::errs());
        ...
        if (failed(getInterfaceVariables(op, interfaceVars))) {return;}

        builder.create<spirv::EntryPointOp>(op.getLoc(), spirv::ExecutionModel::GLCompute, op, interfaceVars);

        builder.create<spirv::ExecutionModeOp>(op.getLoc(), op, spirv::ExecutionMode::SubgroupSize, threadsPerWarp);
  ...
  if (!linkExternLib(linked_binary, externLibs))return failure();

  if (!optimizeSPIRVModule(linked_binary))
    return failure();

  if (failed(disassembleSPIRV(linked_binary.data(), linked_binary.size(), output))) return failure();
...
std::string translateTritonGPUToSPIRVIR(mlir::ModuleOp module, std::map<std::string, int> computeCapability) {
  ...
  std::cout << "\033[1;34m" << spirvModule << "\033[0m" << std::endl;
    return spirvModule;
  }
